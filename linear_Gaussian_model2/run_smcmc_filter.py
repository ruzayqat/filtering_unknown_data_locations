...
def main():
    ...
    simu = range(params["nsimu"])

    starttime = time.time()

    Parallel(n_jobs=params["nsimu"])(delayed(mcmc_filter)(params, h) for h in simu)

    ##if you run the following nsumil > logical cores (like 4 times more), writing to files will only write for half the or 1/3 the nsimuls. 
