...
class SWE_Solver():
    ...
    def _return_sol(self, prev_sol, init_time):
...
def solve(num, vec_in, params):
    ...
    if SOLVER is None:
        _init_SOLVER(params)
    if num == 1:
        tstep, sol = SOLVER._return_sol(vec_in, params["time"])
    else:
        ...
        if params["run_solver_in_parallel"]:
            dt, sol = zip(*Parallel(n_jobs=params["ncores"])(delayed(SOLVER._return_sol)(row,params["time"])
                for row in vec_in.T
                ))
            dt = np.array(dt)
            sol = np.array(sol).T
            ...
            for i in range(num):
                dt, sol[:, i] = SOLVER._return_sol(vec_in[:, i],params["time"])
                tstep += dt
            tstep /= num
    return tstep, sol